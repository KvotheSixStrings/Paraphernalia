...
public class Damage : MonoBehaviour {
    ...
    public bool applyToAncestors = false;
	...
	void OnTriggerEnter2D (Collider2D collider) {
		...
		if (h == null && applyToAncestors) h = collider.gameObject.GetAncestorComponent<HealthController>();
		if (h != null) h.TakeDamage(GetDamage());
	}

	void OnCollisionEnter2D (Collision2D collision) {
		HealthController h = collision.gameObject.GetComponent<HealthController>();
		if (h == null && applyToAncestors) h = collision.gameObject.GetAncestorComponent<HealthController>();
